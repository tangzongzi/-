...
echarts.init(document.getElementById('chart-daily'), 'light').setOption({
  ...
  legend: { data: ['日订单数', '日金额(元)'], textStyle: { color: textColor }, top: 0 },
  grid: { left: 50, right: 60, top: 35, bottom: 60 },
  xAxis: { type: 'category', data: DATA.sales.daily.dates, axisLabel: { color: textColor, rotate: 45, fontSize: 9 }, axisLine: { lineStyle: { color: '#e8e8e8' } } },
  yAxis: [
  ...
  series: [
    { name: '日订单数', type: 'bar', data: DATA.sales.daily.order, itemStyle: { color: C.blue }, barWidth: '60%' },
    { name: '日金额(元)', type: 'line', yAxisIndex: 1, data: DATA.sales.daily.amount, smooth: true, lineStyle: { color: C.orange, width: 2 }, itemStyle: { color: C.orange }, areaStyle: { color: 'rgba(251,146,60,0.15)' } }
  ]
});
...
async function loadDataAndRender() {
  const DATA = await fetch('../../数据/看板数据/销售业绩_日度.json').then(r => r.json());
